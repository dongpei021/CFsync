// Problem: C - Dima and Containers
// Platform: codeforces
// Contest: Contest-358
// Rating/Difficulty: 2000
// Language: C++20 (GCC 13-64)
// Verdict: Accepted
// URL: https://codeforces.com/contest/358/submission/387976136
// Solved on: 2026-08-22T08:33:40.946Z
...
int32_t main() {
    ...
    ivi(op, n);
    vvi cmd(n);
    vector<string> cmdin{"pushStack", "pushQueue", "pushFront", "pushBack"};
    vector<string> cmdout{"popStack", "popQueue", "popFront", "popBack"};
    FOR(i, n) {
        multiset<int> st;
        FOR(j, i, n - 1) {
            if (op[j] == 0) {
    ...
    FOR(i, n) {
        if (op[i] == 0) {
            ...
            cout << cmdin[cmd[i].size() ? cmd[i][0] : 0] << nl;
        }
    }
}